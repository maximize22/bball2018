...
br.table <- as.data.frame(br.extr$expanded_standings_overall)

tgt.date <- Sys.Date()

today.url <- set.dir.url(tgt.date)
### USE TOMORROW FOR EVERYTHING
tomorrow.url <- set.dir.url(tgt.date + 1)

### get list of today's guids
filenames = getURL(tomorrow.url, ftp.use.epsv = FALSE, dirlistonly = TRUE)
...
###once games have started, scoreboard.xml does NOT have pitching info anymore
...
vegas.url <- set.vegas.url('http://www.vegasinsider.com/mlb/matchups/matchups.cfm/date/',(tgt.date + 1))
...
names(tables)
xx <- readHTMLTable(doc)

vegas.url <- 'http://www.oddsshark.com/mlb/odds'
vegas.extr <- getURL(vegas.url)
xmltop <- xmlRoot(xmlfile)
tgt.game <- xmlToList(xmltop)
doc.html = xmlTreeParse(vegas.url,                         useInternal = TRUE)
