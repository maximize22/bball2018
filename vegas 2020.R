...
d.list <- as.data.frame(seq(from=as.Date("2018-03-29"),to=as.Date("2018-07-15"),by="days"))  #2016-07-10 is the last day before all-star break
d.list <- as.data.frame(seq(from=as.Date("2018-07-19"),to=as.Date("2018-09-30"),by="days"))  #2016-07-10 is the last day before all-star break
...
d.list <- as.data.frame(seq(from=as.Date("2019-03-28"),to=as.Date("2019-07-07"),by="days"))  #2016-07-10 is the last day before all-star break
d.list <- as.data.frame(seq(from=as.Date("2019-07-11"),to=as.Date("2019-09-29"),by="days"))  #2016-07-10 is the last day before all-star break
#d.list <- as.data.frame(seq(from=as.Date("2019-08-07"),to=as.Date("2019-08-08"),by="days"))  #2016-07-10 is the last day before all-star break
#07-07
...
all.data <- ddply(d.list, c("t.dates"), .fun = function(x) {
  ...
  print(x$t.dates)
  t.row <- nrow(t1)
  pitching.data <- as.data.frame(t1[49:t.row,],stringsAsFactors = FALSE) #was 52+]
  ...
  pd2$win <- ifelse(grepl("\u00AB", pd2$teams),"W","L")
  pd2$teams <- gsub("\u00AB","", pd2$teams, fixed=TRUE)
  ###sure this could be done quicker with plyr
  pd2$teams <- gsub("\u00C2","", pd2$teams)
  pd2$teams <- gsub("\u00C3","", pd2$teams)
  pd2$teams <- gsub("\\‚"," ", pd2$teams)
  pd2$pitchers <- gsub("Â"," ", pd2$pitchers)
  pd2$WL <- gsub("Â"," ", pd2$WL)
  pd2$Streak <- gsub("Â"," ", pd2$Streak)
  pd2$ML <- gsub("Â"," ", pd2$ML)
  ...
  pd2$teams <- substr(pd2$teams,5,nchar(pd2$teams))
  ...
  #sometimes valid games have no ATS
  pd3 <- subset(pd3, pd3$home.pitcher != ' ')
  return(pd3)
  }
)

#bb.data <- b2019
bb.data <- rbind(bb.data, all.data)
b19 <- bb.data
save(b19, file=paste0(getwd(),"/vegas2019.Rdata"))


load(file=paste0(getwd(),"/vegas2018.Rdata"))
...
bb.data18 <- bb.data # rename for 2017

save(bb.data18, file=paste0(getwd(),"/vegas2018.Rdata"))
